...
const VehicleSchedulePage = () => {
  const { user } = useAuth()
  const { schedules, updateSchedules, updateSchedule } = useGlobalState()
  ...
  // Không cần loadSchedules nữa vì dữ liệu sẽ được cập nhật tự động từ global state
  useEffect(() => {
    if (user) {
  ...
  const handleCancelSchedule = async (scheduleId) => {
    ...
    try {
      ...
      if (response.success) {
        // Reload schedules
        updateSchedule(scheduleId, { trang_thai: 'da_huy' })
  ...
  // Hàm để load schedules khi cần thiết (ví dụ: khi click nút làm mới)
  ...
  // Load schedules khi activeTab thay đổi
  useEffect(() => {
    if (user && activeTab) {
      loadSchedules()
    }
  ...
  return (
    <div className="min-h-screen bg-gray-25">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800 mb-6 sm:mb-8">Quản lý lịch xe</h1>
        
        {/* Tabs */}
        <div className="bg-white mb-4 sm:mb-6 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="bg-white mb-4 sm:mb-6 rounded-xl shadow-sm p-4 sm:p-6">
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-2.5 border border-gray-200 rounded-lg sm:rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-xs sm:text-sm bg-gray-50"
                />
                <SearchOutlined className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-xs sm:text-sm" />
              </div>
            </div>
            ...
            <div className="sm:col-span-2 lg:col-span-1">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <input
                  ...
                  className="px-2 sm:px-3 py-2 sm:py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-xs sm:text-sm bg-gray-50"
                />
                <input
                  type="date"
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-3 sm:space-y-0">
            ...
            <div className="flex items-center space-x-3">
              <span className="text-xs sm:text-sm text-gray-500">{filteredSchedules.length} lịch xe</span>
              <button
                onClick={loadSchedules}
                disabled={loading}
                className="px-3 sm:px-4 py-2 text-xs sm:text-sm bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors disabled:opacity-50"
              >
          ...
          {loading ? (
            <div className="flex items-center justify-center py-8 sm:py-12">
              <LoadingOutlined className="text-2xl sm:text-3xl text-blue-500 animate-spin mr-3" />
              <span className="text-sm sm:text-base text-gray-500">Đang tải danh sách lịch xe...</span>
            </div>
          ) : error ? (
            <div className="flex items-center justify-center py-8 sm:py-12">
              <ExclamationCircleOutlined className="text-2xl sm:text-3xl text-red-500 mr-3" />
              <span className="text-sm sm:text-base text-red-500">{error}</span>
            </div>
          ) : filteredSchedules.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              <CarOutlined className="text-3xl sm:text-4xl text-gray-300 mb-4" />
              <p className="text-sm sm:text-base text-gray-500">Không có lịch xe nào</p>
            </div>
          ) : (
            <>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start space-x-4">
                          {/* Schedule Type Icon */}
                          <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-500 rounded-xl flex items-center justify-center text-white shadow-sm flex-shrink-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between mb-2">
                              ...
                              <div className="flex items-center space-x-2 ml-4">
                                <span className={`px-3 py-1 rounded-lg text-xs font-medium border ${statusInfo.color}`}>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start space-x-3">
                          {/* Schedule Type Icon */}
                          <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-blue-500 rounded-xl flex items-center justify-center text-white shadow-sm flex-shrink-0">
                        ...
                        {/* Action Buttons */}
                        {canCancel && (
                          <div className="flex flex-col items-stretch space-y-2 mt-3 pt-3 border-t border-gray-100">
                            {/* Thông báo về quyền hủy */}
                            ...
                            <button
                              onClick={() => handleCancelSchedule(schedule.id_lich_xe)}
                              className="flex items-center justify-center space-x-2 px-3 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-xs font-medium"
                            >
